...
- **Opt-in `entrypoint-bridge` extension: headless `--resume` forks now ride the interactive session's prompt cache.** Measured on Claude Code 2.1.221: `claude -p --resume <sid> --fork-session` replays the recorded conversation byte-identically (message history, SessionStart hook outputs, attachments — all verified equal), but the headless entrypoint builds a different static prefix — billing header `cc_entrypoint=sdk-cli` vs `cli`, a different identity line (`"You are a Claude agent, built on Anthropic's Claude Agent SDK."` vs `"You are Claude Code, Anthropic's official CLI for Claude."`), different `# Session-specific guidance` / `# Scratchpad Directory` sections in the third system block, and a different tool roster (interactive-only tools such as `Artifact`/`AskUserQuestion`/`EnterPlanMode`/`ExitPlanMode`/`SendUserFile` are absent, and mid-session ToolSearch-deferred MCP tools are re-declared). Tools serialize first in the cache prefix, so the divergence lands at the second tool and the entire conversation body re-prefills on every headless resume. With `CACHE_FIX_ENTRYPOINT_BRIDGE=1`, the proxy persists the interactive prefix shape (tools + system texts, keyed by a hash of the recorded first message, so a snapshot binds only to its own session lineage) and substitutes it into matching `sdk-cli` requests. Measured effect (4-turn interactive session, haiku): fork `cache_read` 32,324 → **59,172 of a 59,168-token prefix (100%)**, fork `cache_creation` 20,101 → **179**; a second fork of the same session then reads 59,351 with **0** creation. Guards: same `cc_version`, same model, same system-block count, snapshot fresh (default 2h, `CACHE_FIX_ENTRYPOINT_BRIDGE_MAX_AGE_MS`), fail-open no-op otherwise; only `sdk-cli` requests are ever bridged (never `sdk-ts`/`sdk-py`). **Default OFF** because the bridge re-declares interactive-only tool schemas to a headless client: if the model calls one (e.g. `AskUserQuestion`), the client sees a `tool_use` it did not declare and answers with an unknown-tool error result — one wasted round-trip on an opt-in path, in exchange for not re-prefilling the whole session.
